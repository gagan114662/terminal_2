...
class FeedbackItem:
    ...
    def __post_init__(self):
        ...
        if self.id is None:
            ...
            content = f"{self.title}{self.description}{self.file_path}{self.line_number}"
...
class FeedbackParser:
    ...
    def parse_user_corrections(self, corrections: List[Dict[str, Any]]) -> List[FeedbackItem]:
    ...
    def _categorize_github_comment(self, comment_text: str) -> FeedbackType:
        ...
        if any(keyword in comment_lower for keyword in ['security', 'vulnerability', 'exploit']):
            return FeedbackType.SECURITY_CONCERN
        elif any(keyword in comment_lower for keyword in ['performance', 'slow', 'optimize', 'efficient']):
            return FeedbackType.PERFORMANCE_ISSUE
        elif any(keyword in comment_lower for keyword in ['style', 'format', 'lint', 'convention']):
            return FeedbackType.STYLE_VIOLATION
        elif any(keyword in comment_lower for keyword in ['design', 'architecture', 'pattern']):
    ...
    def _assess_severity(self, comment_text: str) -> FeedbackSeverity:
        ...
        if any(keyword in comment_lower for keyword in ['critical', 'blocker', 'must fix', 'breaking']):
            return FeedbackSeverity.CRITICAL
        elif any(keyword in comment_lower for keyword in ['important', 'should fix', 'high priority']):
            return FeedbackSeverity.HIGH
        elif any(keyword in comment_lower for keyword in ['nit', 'minor', 'suggestion', 'consider']):
...
class PatternLearner:
    ...
    def learn_from_feedback(self, feedback_items: List[FeedbackItem]) -> List[FeedbackPattern]:
    ...
    def _group_similar_feedback(self, feedback_items: List[FeedbackItem]) -> List[List[FeedbackItem]]:
    ...
    def _extract_pattern(self, feedback_group: List[FeedbackItem]) -> Optional[FeedbackPattern]:
        ...
        common_words = self._find_common_words([item.description for item in feedback_group])
        ...
        confidence_score = min(len(feedback_group) / 10.0, 1.0)  # Max confidence at 10+ occurrences
        ...
        pattern = FeedbackPattern(
            ...
            examples=[item.description for item in feedback_group[:3]]  # First 3 examples
        )
    ...
    def _find_common_words(self, texts: List[str]) -> List[str]:
        ...
        return [word for word in common_words if word not in stop_words and len(word) > 2]
    ...
    def _extract_conditions(self, feedback_group: List[FeedbackItem]) -> List[str]:
        ...
        code_snippets = [item.code_snippet for item in feedback_group if item.code_snippet]
        if code_snippets:
        ...
        if all_tags:
            ...
            common_tags = [tag for tag, count in Counter(all_tags).most_common(3) if count > 1]
            if common_tags:
    ...
    def _find_code_patterns(self, code_snippets: List[str]) -> List[str]:
        ...
        if function_calls:
            from collections import Counter
            common_functions = [func for func, count in Counter(function_calls).most_common(3) if count > 1]
            if common_functions:
    ...
    def _generate_pattern_description(self, feedback_group: List[FeedbackItem]) -> str:
        ...
        if common_words:
            description = f"{feedback_type} pattern related to {', '.join(common_words[:3])}"
        else:
            description = f"{feedback_type} pattern with {len(feedback_group)} occurrences"

        return description

    def _merge_patterns(self, existing: FeedbackPattern, new: FeedbackPattern) -> FeedbackPattern:
        ...
        merged = FeedbackPattern(
            ...
            confidence_score=min((existing.confidence_score + new.confidence_score) / 2, 1.0),
            occurrences=existing.occurrences + new.occurrences,
            last_seen=max(existing.last_seen, new.last_seen),
            examples=list(set(existing.examples + new.examples))[:5]  # Keep top 5 examples
        )
    ...
    def get_relevant_patterns(self, context: Dict[str, Any]) -> List[FeedbackPattern]:
        ...
        relevant_patterns.sort(
            key=lambda p: (p.confidence_score, p.last_seen),
            reverse=True
        )

        return relevant_patterns

    def _is_pattern_relevant(self, pattern: FeedbackPattern, context: Dict[str, Any]) -> bool:
...
class FeedbackEngine:
    ...
    def __init__(self, storage_path: str = None):
        ...
        """
        self.storage_path = Path(storage_path) if storage_path else Path.cwd() / '.termnet' / 'feedback'
    ...
    def ingest_user_corrections(self, corrections: List[Dict[str, Any]]) -> List[FeedbackItem]:
    ...
    def get_feedback_summary(self, days: int = 30) -> FeedbackSummary:
        ...
        improvement_suggestions = self._generate_improvement_suggestions(recent_feedback, top_patterns)

        # Calculate overall confidence
        if top_patterns:
            confidence_score = sum(p.confidence_score for p in top_patterns) / len(top_patterns)
        else:
    ...
    def _load_recent_feedback(self, cutoff_date: datetime) -> List[FeedbackItem]:
        ...
        for feedback_file in feedback_dir.glob('*.json'):
            try:
                ...
                try:
                    item_timestamp = item.timestamp.replace('Z', '+00:00') if 'Z' in item.timestamp else item.timestamp
                ...
                self.logger.warning(f"Failed to load feedback from {feedback_file}: {e}")

        return feedback_items

    def _generate_improvement_suggestions(self, recent_feedback: List[FeedbackItem],
                                        top_patterns: List[FeedbackPattern]) -> List[str]:
        ...
        if recent_feedback:
            ...
            suggestions.append(f"Focus on reducing {most_common_type.value.replace('_', ' ')} issues")
        ...
        high_severity_count = sum(1 for item in recent_feedback
                                if item.severity in [FeedbackSeverity.HIGH, FeedbackSeverity.CRITICAL])

        if high_severity_count > 5:
            suggestions.append("Implement additional pre-commit checks to catch high-severity issues")
