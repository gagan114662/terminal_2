...
class GitClient:
    ...
    def commit(self, message: str, details: str = "", files: List[str] = None) -> GitResult:
        """
...
class PRClient:
    ...
    def _run_gh_command(self, args: List[str]) -> GitResult:
        ...
        try:
            ...
            return GitResult(
                success=False,
                message=f"GitHub CLI command failed: {e}",
                return_code=-1
            )

    def create_pr(self, title: str, body: str = "", base_branch: str = "main") -> GitResult:
        """
        ...
        if not body:
            ...
            body = self.config["pr_template"].format(
                summary=title,
                changes=f"- Modified {len(changed_files)} files" if changed_files else "- No file changes detected",
    ...
    def update_pr(self, pr_number: int, title: str = None, body: str = None) -> GitResult:
        """
...
class RepoOperations:
    ...
    def create_feature_branch(self, feature_name: str, base_branch: str = "main") -> GitResult:
        """
    ...
    def commit_changes(self, summary: str, details: str = "", files: List[str] = None) -> GitResult:
        """
    ...
    def create_pr_for_branch(self, title: str, description: str = "", base_branch: str = "main") -> Tuple[GitResult, GitResult]:
        """
        ...
        if not push_result.success:
            return push_result, GitResult(
                success=False,
                message="Skipped PR creation due to push failure"
            )
