...
class WorkPlanner:
    ...
    def changeplan_md(self, goal: str, task_graph: Dict[str, Any], tests: List[TestCaseSpec]) -> str:
        """
...
"""
    ...
    def _decompose_goal(self, goal: str, repo_intel: Dict[str, Any], plan_hash: str) -> List[TaskNode]:
        ...
        if any(word in goal_lower for word in ["fix", "bug", "error", "issue"]):
            tasks.append(TaskNode(
                ...
                estimated_files=repo_intel.get("files", [])[:3]  # Estimate top files
            ))
        ...
        tasks.append(TaskNode(
            ...
            dependencies=[task.id for task in tasks[1:]],  # Depends on all implementation tasks
            risk="medium",
            done_criteria=[
                "All tests passing",
                "No breaking changes",
    ...
    def _topological_sort(self, nodes: Dict[str, Any], edges: List[Dict[str, str]]) -> List[str]:
