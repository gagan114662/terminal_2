...
class EditEngine:
    ...
    def _parse_unified_diff(self, diff: str) -> Dict[str, List[Dict[str, Any]]]:
        ...
        while i < len(lines):
            ...
            # Hunk content
            elif current_hunk is not None and (line.startswith(' ') or line.startswith('-') or line.startswith('+') or line == ''):
    ...
    def _check_guardrails(self, patches: Dict[str, List[Dict[str, Any]]]) -> List[GuardrailViolation]:
    ...
    def _can_apply_hunk(self, content: List[str], hunk: Dict[str, Any]) -> bool:
        ...
        for line in hunk["lines"]:
            if line.startswith(' '):
                context_lines.append((len(context_lines) + len(removed_lines), line[1:]))
            elif line.startswith('-'):
                removed_lines.append((len(context_lines) + len(removed_lines), line[1:]))
    ...
    def _apply_patches_with_retry(self, patches: Dict[str, List[Dict[str, Any]]], original_diff: str) -> EditResult:
    ...
    def _apply_hunks_to_content(self, content: List[str], hunks: List[Dict[str, Any]]) -> List[str]:
    ...
    def _retry_with_split_hunks(self, patches: Dict[str, List[Dict[str, Any]]], conflicts: List[str]) -> List[str]:
        ...
        for conflict in conflicts:
            if ":hunk" in conflict:
                ...
                if file_path in patches and hunk_index < len(patches[file_path]):
                    ...
                    if self._try_split_hunk_application(file_path, patches[file_path][hunk_index]):
    ...
    def _create_backup(self, file_paths: List[str]):
        ...
        for file_path in file_paths:
            if os.path.exists(file_path):
                backup_path = os.path.join(self._backup_dir, file_path.replace('/', '_'))
